...
/// The core packet structure for all EdgePulse protocol messages.
/// Encapsulates all data needed for routing through the mesh network,
/// including TTL for hop limiting and optional encryption.
/// - Note: Packets larger than BLE MTU (512 bytes) are automatically fragmented
struct EdgepulsePacket: Codable {
    let version: UInt8
    let type: UInt8
    let senderID: Data
    ...
    func toBinaryDataForSigning() -> Data? {
        ...
        let unsignedPacket = EdgepulsePacket(
            type: type,
            senderID: senderID,
    ...
    static func from(_ data: Data) -> EdgepulsePacket? {

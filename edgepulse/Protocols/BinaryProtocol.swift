...
/// # BinaryProtocol
///
/// Low-level binary encoding and decoding for EdgePulse protocol messages.
...
/// Implements binary encoding and decoding for EdgePulse protocol messages.
/// Provides static methods for converting between EdgepulsePacket objects and
...
struct BinaryProtocol {
    ...
    // Encode EdgepulsePacket to binary format
    static func encode(_ packet: EdgepulsePacket, padding: Bool = true) -> Data? {
    ...
    // Decode binary data to EdgepulsePacket
    static func decode(_ data: Data) -> EdgepulsePacket? {
    ...
    private static func decodeCore(_ raw: Data) -> EdgepulsePacket? {
        guard raw.count >= v1HeaderSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> EdgepulsePacket? in

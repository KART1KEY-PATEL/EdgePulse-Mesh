...
final class GossipSyncManager {
    ...
    private struct PacketStore {
        private(set) var packets: [String: EdgepulsePacket] = [:]
        private(set) var order: [String] = []

        mutating func insert(idHex: String, packet: EdgepulsePacket, capacity: Int) {
        ...
        func allPackets(isFresh: (EdgepulsePacket) -> Bool) -> [EdgepulsePacket] {
        ...
        mutating func remove(where shouldRemove: (EdgepulsePacket) -> Bool) {
        ...
        mutating func removeExpired(isFresh: (EdgepulsePacket) -> Bool) {
            remove { !isFresh($0) }
        }
    }
    ...
    private var latestAnnouncementByPeer: [PeerID: (id: String, packet: EdgepulsePacket)] = [:]
    ...
    func onPublicPacketSeen(_ packet: EdgepulsePacket) {
    ...
    private func isPacketFresh(_ packet: EdgepulsePacket) -> Bool {
    ...
    private func isAnnouncementFresh(_ packet: EdgepulsePacket) -> Bool {
    ...
    private func _onPublicPacketSeen(_ packet: EdgepulsePacket) {
    ...
    private func buildGcsPayload(for types: SyncTypeFlags) -> Data {
        var candidates: [EdgepulsePacket] = []

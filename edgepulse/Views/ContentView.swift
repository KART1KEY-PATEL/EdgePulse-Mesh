...
// MARK: - Body
    ...
    private func messagesView(privatePeer: PeerID?, isAtBottom: Binding<Bool>) -> some View {
        ...
        let windowedMessages: [EdgepulseMessage] = Array(messages.suffix(currentWindowCount))
    ...
    private var inputView: some View {
        VStack(alignment: .leading, spacing: 6) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ForEach(Array(viewModel.autocompleteSuggestions.prefix(4)), id: \.self) { suggestion in
                        Button(action: {
                            ...
                            HStack {
                                Text(suggestion)
                                    .font(.edgepulseSystem(size: 11, design: .monospaced))
                                    .foregroundColor(textColor)
                                    .fontWeight(.medium)
                                Spacer()
                            }
            ...
            HStack(alignment: .center, spacing: 4) {
                ...
                .font(.edgepulseSystem(size: 15, design: .monospaced))
                .foregroundColor(textColor)
    ...
    private func handleOpenURL(_ url: URL) {
        guard url.scheme == "edgepulse" else { return }
        switch url.host {
        case "user":
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 12) {
                    Text(peopleSheetTitle)
                        .font(.edgepulseSystem(size: 18, design: .monospaced))
                        .foregroundColor(textColor)
                    Spacer()
                    if case .mesh = locationManager.selectedChannel {
                        Button(action: { showVerifySheet = true }) {
                            Image(systemName: "qrcode")
                                .font(.edgepulseSystem(size: 14))
                        }
                        .buttonStyle(.plain)
                        .help(
                    ...
                    Button(action: {
                        ...
                        Image(systemName: "xmark")
                            .font(.edgepulseSystem(size: 12, weight: .semibold, design: .monospaced))
                ...
                if let subtitle = peopleSheetSubtitle {
                    ...
                    .font(.edgepulseSystem(size: 12, design: .monospaced))
                } else {
                    Text(activeText)
                        .font(.edgepulseSystem(size: 12, design: .monospaced))
                        .foregroundColor(.secondary)
        ...
        Button(action: {
            ...
            HStack(spacing: 6) {
                ...
                Text(context.displayName)
                    .font(.edgepulseSystem(size: 16, weight: .medium, design: .monospaced))
                    .foregroundColor(textColor)
    ...
    private var mainHeaderView: some View {
        HStack(spacing: 0) {
            Text(verbatim: "edgepulse/")
                .font(.edgepulseSystem(size: 18, weight: .medium, design: .monospaced))
                .foregroundColor(textColor)
                .onTapGesture(count: 3) {
            ...
            HStack(spacing: 0) {
                Text(verbatim: "@")
                    .font(.edgepulseSystem(size: 14, design: .monospaced))
                    ...
                    VStack(spacing: 12) {
                        HStack {
                            Text("content.notes.title")
                                .font(.edgepulseSystem(size: 16, weight: .bold, design: .monospaced))
                            Spacer()
                            Button(action: { showLocationNotes = false }) {
                                Image(systemName: "xmark")
                                    .font(.edgepulseSystem(size: 13, weight: .semibold, design: .monospaced))
                                    .foregroundColor(textColor)
                                    .frame(width: 32, height: 32)
                            }
                            .buttonStyle(.plain)
...
private extension ContentView {
    func mediaAttachment(for message: EdgepulseMessage) -> MessageMedia? {
    ...
    func mediaSendState(for message: EdgepulseMessage, mediaURL: URL) -> (isSending: Bool, progress: Double?, canCancel: Bool) {
        var isSending = false
        var progress: Double?
    ...
    @ViewBuilder
    private func messageRow(for message: EdgepulseMessage) -> some View {
        if message.sender == "system" {
    ...
    @ViewBuilder
    private func systemMessageRow(_ message: EdgepulseMessage) -> some View {
    ...
    @ViewBuilder
    private func mediaMessageRow(message: EdgepulseMessage, media: MessageMedia) -> some View {
    ...
    @ViewBuilder
    private func textMessageRow(_ message: EdgepulseMessage) -> some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isLong && cashuTokens.isEmpty {
                ...
                .font(.edgepulseSystem(size: 11, weight: .medium, design: .monospaced))
    ...
    private func expandWindow(ifNeededFor message: EdgepulseMessage,
                              allMessages: [EdgepulseMessage],
                              privatePeer: PeerID?,
                ...
                HStack {
                    Button(action: { dismiss() }) {
                        Text("close", comment: "Button to dismiss fullscreen media viewer")
                            .font(.edgepulseSystem(size: 15, weight: .semibold))

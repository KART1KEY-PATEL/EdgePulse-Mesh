...
struct PublicTimelineStore {
    private var meshTimeline: [EdgepulseMessage] = []
    private var geohashTimelines: [String: [EdgepulseMessage]] = [:]
    ...
    mutating func append(_ message: EdgepulseMessage, to channel: ChannelID) {
        switch channel {
        case .mesh:
    ...
    mutating func append(_ message: EdgepulseMessage, toGeohash geohash: String) {
    ...
    mutating func appendIfAbsent(_ message: EdgepulseMessage, toGeohash geohash: String) -> Bool {
        var timeline = geohashTimelines[geohash] ?? []
    ...
    mutating func messages(for channel: ChannelID) -> [EdgepulseMessage] {
        switch channel {
        case .mesh:
            return meshTimeline
        case .location(let channel):
    ...
    @discardableResult
    mutating func removeMessage(withID id: String) -> EdgepulseMessage? {
    ...
    mutating func removeMessages(in geohash: String, where predicate: (EdgepulseMessage) -> Bool) {
        var timeline = geohashTimelines[geohash] ?? []
        timeline.removeAll(where: predicate)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }

    mutating func mutateGeohash(_ geohash: String, _ transform: (inout [EdgepulseMessage]) -> Void) {
        var timeline = geohashTimelines[geohash] ?? []
        transform(&timeline)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }
    ...
    private func trimGeohashTimelineIfNeeded(_ timeline: inout [EdgepulseMessage]) {

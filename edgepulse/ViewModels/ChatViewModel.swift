...
/// # ChatViewModel
///
/// The central business logic and state management component for EdgePulse.
...
/// SwiftUI views and the underlying EdgePulse services. It manages:
...
/// - **EdgepulseDelegate**: Receives messages and events from BLEService
...
/// Manages the application state and business logic for EdgePulse.
/// Acts as the primary coordinator between UI components and backend services,
/// implementing the EdgepulseDelegate protocol to handle network events.
final class ChatViewModel: ObservableObject, EdgepulseDelegate {
    ...
    @MainActor
    private func normalizedSenderKey(for message: EdgepulseMessage) -> String {
    ...
    @Published var messages: [EdgepulseMessage] = []
    ...
    @Published var allPeers: [EdgepulsePeer] = []
    var privateChats: [PeerID: [EdgepulseMessage]] {
    ...
    private var peerIndex: [PeerID: EdgepulsePeer] = [:]
    ...
    private let nicknameKey = "edgepulse.nickname"
    ...
    init(
        ...
        let peersCancellable = unifiedPeerService.$peers
            ...
            .sink { [weak self] peers in
                ...
                var uniquePeers: [PeerID: EdgepulsePeer] = [:]
                for peer in peers {
    ...
    private func subscribeGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard let (content, senderPubkey, rumorTs) = try? NostrProtocol.decryptPrivateMessage(giftWrap: giftWrap, recipientIdentity: id),
              content.hasPrefix("edgepulse1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("edgepulse1:".count))),
              let packet = EdgepulsePacket.from(packetData),
    ...
    /// Sends a message through the EdgePulse network.
    ...
    private func handleGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard content.hasPrefix("edgepulse1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("edgepulse1:".count))),
              let packet = EdgepulsePacket.from(packetData),
              packet.type == MessageType.noiseEncrypted.rawValue,
    ...
    func blockGeohashUser(pubkeyHexLowercased: String, displayName: String) {
        ...
        if let gh = currentGeohash {
            let predicate: (EdgepulseMessage) -> Bool = { [self] msg in
    ...
    func sendVoiceNote(at url: URL) {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            do {
                ...
                let packet = EdgepulseFilePacket(
    ...
    @MainActor
    private func enqueueMediaMessage(content: String, targetPeer: PeerID?) -> EdgepulseMessage {
        let timestamp = Date()
        let message: EdgepulseMessage

        if let peerID = targetPeer {
            message = EdgepulseMessage(
                sender: nickname,
                content: content,
                timestamp: timestamp,
                isRelay: false,
    ...
    private func cleanupLocalFile(forMessage message: EdgepulseMessage) {
    ...
    private func removeMessage(withID messageID: String, cleanupFile: Bool = false) {
        var removedMessage: EdgepulseMessage?
    ...
    func addLocalPrivateSystemMessage(_ content: String, to peerID: PeerID) {
        let systemMessage = EdgepulseMessage(
            sender: "system",
            content: content,
            timestamp: Date(),
            isRelay: false,
                ...
                if let nostrMessages = privateChats[noiseKeyHex], !nostrMessages.isEmpty {
                    ...
                    for message in nostrMessages {
                        if !existingMessageIds.contains(message.id) {
                            ...
                            let updatedMessage = EdgepulseMessage(
                                id: message.id,
    ...
    @objc private func handleNostrMessage(_ notification: Notification) {
        guard let message = notification.userInfo?["message"] as? EdgepulseMessage else { return }
    ...
    @objc private func userDidTakeScreenshot() {
        ...
        if let peerID = selectedPrivateChatPeer {
            ...
            let localNotification = EdgepulseMessage(
                sender: "system",
    ...
    @MainActor
    func getPrivateChatMessages(for peerID: PeerID) -> [EdgepulseMessage] {
        var combined: [EdgepulseMessage] = []
        ...
        var bestByID: [String: EdgepulseMessage] = [:]
        for msg in combined {
    ...
    func panicClearAllData() {
        ...
        userDefaults.removeObject(forKey: "edgepulse.noiseIdentityKey")
        userDefaults.removeObject(forKey: "edgepulse.messageRetentionKey")
    ...
    @MainActor
    func formatMessageAsText(_ message: EdgepulseMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            senderStyle.font = .edgepulseSystem(size: 14, weight: fontWeight, design: .monospaced)
            // Make sender clickable: encode senderPeerID into a custom URL
            if let spid = message.senderPeerID, let url = URL(string: "edgepulse://user/\(spid.toPercentEncoded())") {
            ...
            if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
                ...
                plainStyle.font = isSelf
                    ? .edgepulseSystem(size: 14, weight: .bold, design: .monospaced)
                    : .edgepulseSystem(size: 14, design: .monospaced)
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        ...
                        mentionStyle.font = .edgepulseSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                        ...
                        if type == "hashtag" {
                            // If the hashtag is a valid geohash, make it tappable (edgepulse://geohash/<gh>)
                            ...
                            if isGeohash && !attachedToMention && standalone, let url = URL(string: "edgepulse://geohash/\(token)") {
                                tagStyle.link = url
                            ...
                            matchStyle.font = .edgepulseSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                            if type == "url" {
            ...
            timestampStyle.font = .edgepulseSystem(size: 10, design: .monospaced)
            ...
            contentStyle.font = .edgepulseSystem(size: 12, design: .monospaced).italic()
    ...
    @MainActor
    func formatMessageHeader(_ message: EdgepulseMessage, colorScheme: ColorScheme) -> AttributedString {
        let isSelf: Bool = {
            if let spid = message.senderPeerID {
        ...
        if message.sender == "system" {
            ...
            style.font = .edgepulseSystem(size: 14, weight: .medium, design: .monospaced)
        ...
        var senderStyle = AttributeContainer()
        senderStyle.foregroundColor = baseColor
        senderStyle.font = .edgepulseSystem(size: 14, weight: isSelf ? .bold : .medium, design: .monospaced)
        if let spid = message.senderPeerID,
           let url = URL(string: "edgepulse://user/\(spid.id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid.id)") {
            senderStyle.link = url
        }
    ...
    func formatMessage(_ message: EdgepulseMessage, colorScheme: ColorScheme) -> AttributedString {
        var result = AttributedString()
        ...
        if message.sender == "system" {
            ...
            senderStyle.font = .edgepulseSystem(size: 12, weight: fontWeight, design: .monospaced)
                ...
                if let range = Range(match.range(at: 0), in: contentText) {
                    if lastEndIndex < range.lowerBound {
                        ...
                        if !beforeText.isEmpty {
                            ...
                            normalStyle.font = .edgepulseSystem(size: 14, design: .monospaced)
                    ...
                    var mentionStyle = AttributeContainer()
                    mentionStyle.font = .edgepulseSystem(size: 14, weight: .semibold, design: .monospaced)
            ...
            if message.isRelay, let originalSender = message.originalSender {
                ...
                relayStyle.font = .edgepulseSystem(size: 11, design: .monospaced)
    ...
    @MainActor
    private func peerColor(for message: EdgepulseMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID {
            if spid.isGeoChat || spid.isGeoDM {
    ...
    private func addMessage(_ message: EdgepulseMessage) {
    ...
    func getPeer(byID peerID: PeerID) -> EdgepulsePeer? {
    ...
    // MARK: - EdgepulseDelegate Methods
    ...
    func didUpdatePeerList(_ peers: [PeerID]) {
        ...
        DispatchQueue.main.async {
            ...
            // Smart notification logic for "edgepulseters nearby"
            ...
            if meshPeerSet.isEmpty {
                ...
                if !newPeers.isEmpty {
                    ...
                    SecureLogger.info(
                        "👥 Sent edgepulseters nearby notification for \(meshPeers.count) mesh peers (new: \(newPeers.count))",
                        category: .session
                    )
    ...
    private func updateMessageDeliveryStatus(_ messageID: String, status: DeliveryStatus) {
        ...
        for (peerID, chatMessages) in privateChats {
            ...
            // Update delivery status directly (EdgepulseMessage is a class/reference type)
    ...
    private func processNostrMessage(_ giftWrap: NostrEvent) {
        ...
        do {
            ...
            // Expect embedded EdgePulse packet content
            guard content.hasPrefix("edgepulse1:") else {
                SecureLogger.debug("Ignoring non-embedded Nostr DM content", category: .session)
                return
            }

            guard let packetData = Self.base64URLDecode(String(content.dropFirst("edgepulse1:".count))),
                  let packet = EdgepulsePacket.from(packetData) else {
                SecureLogger.error("Failed to decode embedded EdgePulse packet from Nostr DM", category: .session)
                return
            }
    ...
    private func addMessageToPrivateChatsIfNeeded(_ message: EdgepulseMessage, targetPeerID: PeerID) {
    ...
    @MainActor
    private func mirrorToEphemeralIfNeeded(_ message: EdgepulseMessage, targetPeerID: PeerID, key: Data?) {
        guard let key,
    ...
    @MainActor
    private func sendDeliveryAckViaNostrEmbedded(_ message: EdgepulseMessage, wasReadBefore: Bool, senderPubkey: String, key: Data?) {
    ...
    @MainActor
    private func handleViewingThisChat(_ message: EdgepulseMessage, targetPeerID: PeerID, key: Data?, senderPubkey: String) {
    ...
    @MainActor
    private func isMessageBlocked(_ message: EdgepulseMessage) -> Bool {
    ...
    private func processActionMessage(_ message: EdgepulseMessage) -> EdgepulseMessage {
    ...
    private func migratePrivateChatsIfNeeded(for peerID: PeerID, senderNickname: String) {
        ...
        if privateChats[peerID] == nil || privateChats[peerID]?.isEmpty == true {
            var migratedMessages: [EdgepulseMessage] = []
    ...
    @MainActor
    private func handlePrivateMessage(_ message: EdgepulseMessage) {
    ...
    @MainActor
    private func handlePublicMessage(_ message: EdgepulseMessage) {
...
private func checkForMentions(_ message: EdgepulseMessage) {
...
/// Send haptic feedback for special messages (iOS only)
    private func sendHapticFeedback(for message: EdgepulseMessage) {
        #if os(iOS)
        guard UIApplication.shared.applicationState == .active else { return }
...
extension ChatViewModel: PublicMessagePipelineDelegate {
    func pipelineCurrentMessages(_ pipeline: PublicMessagePipeline) -> [EdgepulseMessage] {
        messages
    }

    func pipeline(_ pipeline: PublicMessagePipeline, setMessages messages: [EdgepulseMessage]) {
    ...
    func pipelinePrewarmMessage(_ pipeline: PublicMessagePipeline, message: EdgepulseMessage) {

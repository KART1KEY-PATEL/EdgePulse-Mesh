...
/// - Emits events exclusively via `EdgepulseDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private var pendingDirectedRelays: [PeerID: [String: (packet: EdgepulsePacket, enqueuedAt: Date)]] = [:]
    ...
    func sendMessage(_ content: String, mentions: [String] = [], to recipientID: PeerID? = nil, messageID: String? = nil, timestamp: Date? = nil) {
        ...
        let basePacket = EdgepulsePacket(
    ...
    weak var delegate: EdgepulseDelegate?
    ...
    func stopServices() {
        ...
        let leavePacket = EdgepulsePacket(
    ...
    func sendFileBroadcast(_ filePacket: EdgepulseFilePacket, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendFilePrivate(_ filePacket: EdgepulseFilePacket, to peerID: PeerID, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
            guard let payload = filePacket.encode() else {
    ...
    private func broadcastPacket(_ packet: EdgepulsePacket, transferId: String? = nil) {
    ...
    private func sendEncrypted(_ packet: EdgepulsePacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: EdgepulsePacket, data: Data, pad: Bool) {
    ...
    private func sendOnAllLinks(packet: EdgepulsePacket, data: Data, pad: Bool, directedOnlyPeer: PeerID?) {
    ...
    private func sendPacketDirected(_ packet: EdgepulsePacket, to peerID: PeerID) {
    ...
    private func spoolDirectedPacket(_ packet: EdgepulsePacket, recipientPeerID: PeerID) {
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(String, EdgepulsePacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(String, EdgepulsePacket)] = []
            let now = Date()
    ...
    private func rebroadcastRecentAnnounces() {
        ...
        let packets: [EdgepulsePacket] = collectionsQueue.sync {
    ...
    private func handleFileTransfer(_ packet: EdgepulsePacket, from peerID: PeerID) {
        ...
        guard let filePacket = EdgepulseFilePacket.decode(packet.payload) else {
    ...
    private func handleLeave(_ packet: EdgepulsePacket, from peerID: PeerID) {
...
extension BLEService: GossipSyncManager.Delegate {
    func sendPacket(_ packet: EdgepulsePacket) {
        broadcastPacket(packet)
    }

    func sendPacket(to peerID: PeerID, packet: EdgepulsePacket) {
        sendPacketDirected(packet, to: peerID)
    }

    func signPacketForBroadcast(_ packet: EdgepulsePacket) -> EdgepulsePacket {
...
extension BLEService {
    func _test_handlePacket(_ packet: EdgepulsePacket, fromPeerID: PeerID) {
...
extension BLEService: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        guard let service = services.first(where: { $0.uuid == BLEService.serviceUUID }) else {
            // Not a EdgePulse peer - disconnect
    ...
    private func processNotificationPacket(_ packet: EdgepulsePacket, from peripheral: CBPeripheral, peripheralUUID: String) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        if !hasOurService {
            ...
            SecureLogger.warning("❌ EdgePulse service removed - disconnecting from \(peripheral.name ?? peripheral.identifier.uuidString)", category: .session)
            centralManager?.cancelPeripheralConnection(peripheral)
        } else {
            // Try to rediscover
            peripheral.discoverServices([BLEService.serviceUUID])
...
extension BLEService {
    ...
    private func applyRouteIfAvailable(_ packet: inout EdgepulsePacket, to recipient: PeerID) {
    ...
    private func forwardAlongRouteIfNeeded(_ packet: EdgepulsePacket) -> Bool {
    ...
    private func makeMessageID(for packet: EdgepulsePacket) -> String {
    ...
    private func priority(for packet: EdgepulsePacket, data: Data) -> OutboundPriority {
    ...
    private func sendFragmentedPacket(_ packet: EdgepulsePacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: PeerID? = nil, transferId: String? = nil) {
    ...
    private func startFragmentedPacket(_ context: PendingFragmentTransfer) {
        ...
        for (index, fragment) in fragments.enumerated() {
            ...
            let fragmentPacket = EdgepulsePacket(
    ...
    private func handleFragment(_ packet: EdgepulsePacket, from peerID: PeerID) {
        if DispatchQueue.getSpecific(key: messageQueueKey) != nil {
    ...
    private func _handleFragment(_ packet: EdgepulsePacket, from peerID: PeerID) {
    ...
    private func handleReceivedPacket(_ packet: EdgepulsePacket, from peerID: PeerID) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
    ...
    private func handleAnnounce(_ packet: EdgepulsePacket, from peerID: PeerID) {
    ...
    private func handleRequestSync(_ packet: EdgepulsePacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: EdgepulsePacket, from peerID: PeerID) {
    ...
    private func handleNoiseHandshake(_ packet: EdgepulsePacket, from peerID: PeerID) {
        ...
        if PeerID(hexData: packet.recipientID) == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    var responsePacket = EdgepulsePacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: myPeerIDData,
                        recipientID: Data(hexString: peerID.id),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: EdgepulsePacket, from peerID: PeerID) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [PeerID: [String: (packet: EdgepulsePacket, enqueuedAt: Date)]] = [:]

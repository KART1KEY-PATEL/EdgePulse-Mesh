...
/// peer-to-peer communication in EdgePulse. Acts as the bridge between the transport
...
/// Manages end-to-end encryption for EdgePulse using the Noise Protocol Framework.
...
final class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.edgepulse.noise.service", attributes: .concurrent)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = "edgepulse-announce-v1".data(using: .utf8) ?? Data()
    ...
    /// Sign a EdgepulsePacket using the noise private key
    func signPacket(_ packet: EdgepulsePacket) -> EdgepulsePacket? {
    ...
    /// Verify a EdgepulsePacket signature using the provided public key
    func verifyPacketSignature(_ packet: EdgepulsePacket, publicKey: Data) -> Bool {
...
/// These types wrap the underlying EdgePulse protocol messages with encryption metadata.

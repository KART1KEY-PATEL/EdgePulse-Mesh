...
final class NostrTransport: Transport {
    // Provide BLE short peer ID for EdgePulse embedding
    ...
    func sendPrivateMessage(_ content: String, to peerID: PeerID, recipientNickname: String, messageID: String) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedEdgePulse.encodePMForNostr(content: content, messageID: messageID, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
    ...
    func sendFavoriteNotification(to peerID: PeerID, isFavorite: Bool) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedEdgePulse.encodePMForNostr(content: content, messageID: UUID().uuidString, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
    ...
    func sendDeliveryAck(for messageID: String, to peerID: PeerID) {
        Task { @MainActor in
            ...
            guard let ack = NostrEmbeddedEdgePulse.encodeAckForNostr(type: .delivered, messageID: messageID, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
...
extension NostrTransport {
    ...
    func sendDeliveryAckGeohash(for messageID: String, toRecipientHex recipientHex: String, from identity: NostrIdentity) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedEdgePulse.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID) else { return }
    ...
    func sendReadReceiptGeohash(_ messageID: String, toRecipientHex recipientHex: String, from identity: NostrIdentity) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedEdgePulse.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID) else { return }
            guard let event = try? NostrProtocol.createPrivateMessage(content: embedded, recipientPubkey: recipientHex, senderIdentity: identity) else { return }
            NostrRelayManager.registerPendingGiftWrap(id: event.id)
            NostrRelayManager.shared.sendEvent(event)
        }
    ...
    func sendPrivateMessageGeohash(content: String, toRecipientHex recipientHex: String, from identity: NostrIdentity, messageID: String) {
        Task { @MainActor in
            ...
            // Build embedded EdgePulse packet without recipient peer ID
            guard let embedded = NostrEmbeddedEdgePulse.encodePMForNostrNoRecipient(content: content, messageID: messageID, senderPeerID: senderPeerID) else {
...
extension NostrTransport {
    ...
    private func sendNextReadAck() {
        ...
        Task { @MainActor in
            ...
            guard let ack = NostrEmbeddedEdgePulse.encodeAckForNostr(type: .readReceipt, messageID: item.receipt.originalMessageID, recipientPeerID: item.peerID, senderPeerID: senderPeerID) else {

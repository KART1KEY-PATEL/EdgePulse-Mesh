...
final class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [EdgepulsePeer] = []
    @Published private(set) var connectedPeerIDs: Set<PeerID> = []
    @Published private(set) var favorites: [EdgepulsePeer] = []
    @Published private(set) var mutualFavorites: [EdgepulsePeer] = []
    ...
    private func updatePeers() {
        ...
        var enrichedPeers: [EdgepulsePeer] = []
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: EdgepulsePeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [EdgepulsePeer] = []
        var mutualsList: [EdgepulsePeer] = []
        var newIndex: [PeerID: EdgepulsePeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by peerID: PeerID) -> EdgepulsePeer? {
    ...
    var allPeers: [EdgepulsePeer] { peers }

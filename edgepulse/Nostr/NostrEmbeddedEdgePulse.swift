...
// MARK: - EdgePulse-over-Nostr Adapter

struct NostrEmbeddedEdgePulse {
    /// Build a `edgepulse1:` base64url-encoded EdgePulse packet carrying a private message for Nostr DMs.
    static func encodePMForNostr(content: String, messageID: String, recipientPeerID: PeerID, senderPeerID: PeerID) -> String? {
        ...
        return "edgepulse1:" + base64URLEncode(data)
    }

    /// Build a `edgepulse1:` base64url-encoded EdgePulse packet carrying a delivery/read ack for Nostr DMs.
    ...
    /// Build a `edgepulse1:` ACK (delivered/read) without an embedded recipient peer ID (geohash DMs).
    ...
    /// Build a `edgepulse1:` payload without an embedded recipient peer ID (used for geohash DMs).

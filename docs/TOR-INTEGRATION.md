...
- TorManager
  ...
  - Fails closed by default until Tor is bootstrapped. For local development only, define EDGEPULSE_DEV_ALLOW_CLEARNET to bypass Tor.
- TorURLSession
...
torrc template
The generated torrc (under Application Support/edgepulse/tor/torrc) is:

  DataDirectory <AppSupport>/edgepulse/tor
  ClientOnly 1
...
- To temporarily allow direct network without Tor for local development:
  - Add Swift compiler flag: EDGEPULSE_DEV_ALLOW_CLEARNET

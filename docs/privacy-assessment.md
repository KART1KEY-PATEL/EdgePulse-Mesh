...
- Address: iOS uses BLE MAC randomization; EdgePulse does not attempt to set static addresses.
...
- Payload confidentiality: Messages embed a EdgePulse Noise-encrypted packet inside a NIP-17 gift wrap; relays see only random-looking ciphertext.
...
- Relay policy variance: Some relays apply “web-of-trust” policies and may reject events; EdgePulse tolerates partial delivery and still prefers mesh when available.
...
- Default level: `info`; release builds suppress debug. Developers can set `EDGEPULSE_LOG_LEVEL=debug|info|warning|error|fault`.
- Transport routing, ACK sends, subscribe/connect noise were downgraded from info→debug.
- OS/system errors (e.g., transient WebSocket disconnects) may still appear in system logs; EdgePulse avoids re-logging those unless actionable.

...
struct FragmentationTests {
    ...
    func maxSizedFileTransferSurvivesReassembly() async throws {
        ...
        let filePacket = EdgepulseFilePacket(
            fileName: "limit.bin",
...
extension FragmentationTests {
    private final class CaptureDelegate: EdgepulseDelegate {
        var publicMessages: [(peerID: PeerID, nickname: String, content: String)] = []
        var receivedMessages: [EdgepulseMessage] = []
    ...
    private func makeLargePublicPacket(senderShortHex: PeerID, size: Int) -> EdgepulsePacket {
    ...
    private func fragmentPacket(_ packet: EdgepulsePacket, fragmentSize: Int, fragmentID: Data? = nil, pad: Bool = true) -> [EdgepulsePacket] {

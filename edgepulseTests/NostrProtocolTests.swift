...
struct NostrProtocolTests {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedEdgePulse.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID),
        ...
        // Parse EdgePulse payload
        #expect(content.hasPrefix("edgepulse1:"))
        let base64url = String(content.dropFirst("edgepulse1:".count))
        let packetData = try #require(Self.base64URLDecode(base64url))
        let packet = try #require(EdgepulsePacket.from(packetData), "Failed to decode edgepulse packet")
    ...
    @Test func ackRoundTripNIP44V2_ReadReceipt() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedEdgePulse.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID),

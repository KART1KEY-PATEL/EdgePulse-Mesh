...
public final class SecureLogger {
    ...
    /// Minimum level that will be logged. Defaults to .info. Override via env EDGEPULSE_LOG_LEVEL.
    private static let minimumLevel: LogLevel = {
        let env = ProcessInfo.processInfo.environment["EDGEPULSE_LOG_LEVEL"]?.lowercased()
        switch env {
        case "debug": return .debug

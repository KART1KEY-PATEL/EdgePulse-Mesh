...
public final class TorManager: ObservableObject {
    ...
    // `-DEDGEPULSE_DEV_ALLOW_CLEARNET` to temporarily allow direct network.
    public var torEnforced: Bool {
        #if EDGEPULSE_DEV_ALLOW_CLEARNET
        return false
        #else
        return true
        #endif
    ...
    func dataDirectoryURL() -> URL? {
        do {
            ...
            let dir = base.appendingPathComponent("edgepulse/tor", isDirectory: true)
            return dir
        } catch {
            return nil
        }
